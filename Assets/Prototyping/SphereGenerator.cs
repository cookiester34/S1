...
public class SphereGenerator : MonoBehaviour
{
    ...
    private Mesh GenerateSphereMesh(float radius, int segments)
    {
        ...
        // Create lists to hold vertices, normals, uvs, and triangles
        var vertices = new System.Collections.Generic.List<Vector3>();
        var normals = new System.Collections.Generic.List<Vector3>();
        var uvs = new System.Collections.Generic.List<Vector2>();
        var triangles = new System.Collections.Generic.List<int>();

        // Generate vertices, normals, and UVs for the sphere
        for (int lat = 0; lat <= segments; lat++)
        {
            ...
            for (int lon = 0; lon <= segments; lon++)
            {
                ...
                Vector2 uv = new Vector2((float)lon / segments, (float)lat / segments);
                uvs.Add(uv);
            }
        }
